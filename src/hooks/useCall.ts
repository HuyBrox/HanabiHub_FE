...
export interface UseCallApi {
  ...
  startCallInPopup: (receiverId: string, callType: CallType, role: 'caller' | 'receiver', otherPeerId?: string) => Promise<void>;
  sendPeerIdToReceiver: (receiverId: string, peerId: string, callType: CallType) => void;
  endCall: () => void;
...
export function useCall(): UseCallApi {
  ...
  const initiateCall = useCallback((receiverId: string, callType: CallType) => {
    if (!socket || !connected || !user?._id) {
      console.error('[initiateCall] Socket not ready or user not logged in');
      return;
    }

    console.log('[initiateCall] Starting call notification...', { receiverId, callType });
  ...
  const sendPeerIdToReceiver = useCallback((receiverId: string, peerId: string, callType: CallType) => {
    // Try to use context socket first
    if (socket && connected) {
      console.log('[sendPeerIdToReceiver] Sending real peer ID (context socket)...', { receiverId, peerId, callType });
    ...
    const tryEmit = () => {
      const s = getSocket();
      if (s && s.connected) {
        console.log('[sendPeerIdToReceiver] Sending real peer ID (global socket)...', { receiverId, peerId, callType });
      ...
      if (elapsed >= maxWait) {
        ...
        if (s2) {
          s2.once('connect', () => {
            console.log('[sendPeerIdToReceiver] socket connected (late). Emitting peerId');
    ...
    return new Promise((resolve, reject) => {
      ...
      peer.on("call", (conn) => {
        ...
        conn.on("stream", (remoteStream) => {
          ...
          setState(s => ({ ...s, remoteStream, inCall: true, ringing: false }));
        });
  ...
  const startCallInPopup = useCallback(async (
    receiverId: string, 
    callType: CallType, 
    ...
    try {
      console.log('[startCallInPopup] Starting...', { receiverId, callType, role, otherPeerId });
      ...
      if (role === 'caller') {
        ...
        console.log('[startCallInPopup] Caller sending real peer ID and waiting...', myPeerId);
        ...
        console.log('[startCallInPopup] Receiver calling caller:', otherPeerId);
  ...
  useEffect(() => {
    ...
    const onCallAnswered = (payload: { receiverId: string; accepted: boolean }) => {
