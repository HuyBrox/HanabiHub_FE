...
import styles from "./UpdateLessonModal.module.css";
...
export default function UpdateLessonModal({
  ...
  // Populate form when lesson data changes
  useEffect(() => {
    if (lesson) {
      setFormData({
        title: lesson.title,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/courses/lesson/${lesson._id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (!response.ok) {
        throw new Error("Cập nhật bài học thất bại");
      }
    ...
    <div className={styles.backdrop} onClick={handleClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.header}>
          <h2 className={styles.modalTitle}>Cập nhật bài học</h2>
          <button
